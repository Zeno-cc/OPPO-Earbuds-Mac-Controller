...
- `EarbudsSession` 接收传输事件，将协议报告归并为电量、盒内/盒外位置状态和降噪状态。发送设置后等待耳机回报，不提前修改真实状态。每次电量报告同时记录该槽位的观测时间与连接 generation，供菜单栏判断新鲜度。
- `Buds` 负责 macOS 蓝牙设备发现、自动连接、用户连接意图、系统电量兜底和界面可观察状态。菜单栏的 L/R/C 投影只读取厂商逐槽观测，合并电量与系统兜底值只用于面板和通知。
